...
public class PullRecyclerView extends RecyclerView {
    ...
    public static final int MORE_STATE_RELEASE_TO_REFRESH = 1;
    public static final int MORE_STATE_REFRESHING = 2;
    public static final int MORE_STATE_DONE = 3;
    public static final int MORE_STATE_FAIL = 4;
...
//    }
    ...
    public PullRecyclerView setFeedbackWaitTime(long time) {
        mFeedbackWaitTime = time;
        return this;
    }

    public PullRecyclerView setDragRate(int drag) {
        mDragRate = drag;
        return this;
    }
...
//    }
    ...
    public boolean onTouchEvent(MotionEvent e) {
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isScrollBottom) {
                    if (mMoreRefreshView.getVisibleHeight() == 0 && moveY > 0) {
                        return super.onTouchEvent(e);
                    }
                    if (isOnBottom() && !isRefresh()) {
                        onBottomMove((int) (-moveY / mDragRate));
                        return false;
                    }
                } else if (isCanRefresh()) {
                    if (mHeadRefreshView.getVisibleHeight() == 0 && moveY < 0) {
                        return super.onTouchEvent(e);
                    }
                    if (isOnTop() && !isLoadMore()) {
                        onHeadMove((int) (moveY / mDragRate));
                        return false;
                    }
                }
                break;
    ...
    //判断手势状态
    private void onHeadMove(int move) {
    ...
    private void onBottomMove(int move) {
        if (isCanLoadMore() && !isLoadMore()) {
            int newVisibleHeight = mMoreRefreshView.getVisibleHeight() + move;
            if (newVisibleHeight >= mMoreRefreshView.getRefreshHeight() && mMoreRefreshState != MORE_STATE_RELEASE_TO_REFRESH) {
                mMoreRefreshState = MORE_STATE_RELEASE_TO_REFRESH;
                mMoreRefreshView.onReleaseState();
            }
            if (newVisibleHeight < mMoreRefreshView.getRefreshHeight() && mMoreRefreshState != MORE_STATE_NORMAL) {
                mMoreRefreshState = MORE_STATE_NORMAL;
                mMoreRefreshView.onPullingUp();
            }
            mMoreRefreshView.setVisibleHeight(mMoreRefreshView.getVisibleHeight() + move);
            scrollBy(0, move);
        }
    }

    //设置要执行状态
    private void setHeadRefreshState(int state) {
    ...
    //设置要执行状态
    private void setBottomRefreshState(int state) {
        if (isExistLoadMoreView() && mMoreRefreshState != state) {
            switch (state) {
                case MORE_STATE_REFRESHING://切换到刷新状态
                    mMoreRefreshView.onLoadingMore();
                    mMoreRefreshView.smoothScrollTo(mMoreRefreshView.getRefreshHeight());
                    if (pullListener != null) {
                        pullListener.onLoadMore();
                    }
                    break;
                case MORE_STATE_DONE://切换到刷新完成或者加载成功的状态
                    if (mMoreRefreshState == MORE_STATE_REFRESHING) {
                    ...
                    break;
                case MORE_STATE_FAIL://切换到刷新失败或者加载失败的状态
                    if (mMoreRefreshState == MORE_STATE_REFRESHING) {
    ...
    private boolean isOnBottom() {
        return isExistLoadMoreView() && mMoreRefreshView.getParent() != null;
    }
    ...
    private boolean isScrollBottom() {
        ...
        return layoutManager.getChildCount() > 0 &&
                lastCompletelyVisibleItemPosition >= pullToRefreshRecyclerViewAdapter.getItemCount() - pullViewCount;
    }
    ...
    public void onPullComplete() {
        setHeadRefreshState(HEAD_STATE_DONE);
    }

    //下拉刷新失败
    public void onPullFail() {
        setHeadRefreshState(HEAD_STATE_FAIL);
    }

    //上拉加载成功
    public void onLoadMoreComplete() {
        setBottomRefreshState(MORE_STATE_DONE);
    }

    //上拉加载失败
    public void onLoadMoreFail() {
        setBottomRefreshState(MORE_STATE_FAIL);
    }
